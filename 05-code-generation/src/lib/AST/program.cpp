...
void ProgramNode::visitChildNodeVarDecl(AstNodeVisitor &p_visitor) {
    for (auto &decl : var_decls) {
        decl->accept(p_visitor);
    }
}

void ProgramNode::visitChildNodeFuncDecl(AstNodeVisitor &p_visitor) {
    for (auto &func : funcs) {
        func->accept(p_visitor);
    }
}

void ProgramNode::visitChildNodeBody(AstNodeVisitor &p_visitor) {
    body->accept(p_visitor);
}
