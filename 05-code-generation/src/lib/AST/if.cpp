...
void IfNode::visitChildNodeCond(AstNodeVisitor &p_visitor) {
    condition->accept(p_visitor);
}

void IfNode::visitChildNodeBody(AstNodeVisitor &p_visitor) {
    body->accept(p_visitor);
}

void IfNode::visitChildNodeElseBody(AstNodeVisitor &p_visitor) {
    if (else_body) {
        else_body->accept(p_visitor);
    }
}
