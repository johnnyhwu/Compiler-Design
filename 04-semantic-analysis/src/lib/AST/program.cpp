...
void ProgramNode::visitChildNodesDecl(AstNodeVisitor &p_visitor) {
    for (auto &decl : var_decls) {
        decl->accept(p_visitor);
    }
}

void ProgramNode::visitChildNodesFunc(AstNodeVisitor &p_visitor) {
    for (auto &func : funcs) {
        func->accept(p_visitor);
    }
}

void ProgramNode::visitChildNodesComp(AstNodeVisitor &p_visitor) {
    body->accept(p_visitor);
}
